...
public class AdaptiveTestSampleView extends SampleView {
	...
	private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_jButton1ActionPerformed
		...
		if (jCheckBox1.isSelected()) {
			...
			try {
				if (!jTextPane1.getText().equals("")
						&& !jTextPane2.getText().equals("")) {
					super.elbowUsed = true;
					super.elbowanglemax = Math.toRadians(Integer
							.parseInt(jTextPane2.getText()));
					super.elbowanglemin = Math.toRadians(Integer
							.parseInt(jTextPane1.getText()));
					System.out.println("Set Elbow Angle Range from "
							+ Math.toDegrees(elbowanglemin) + " to "
							+ Math.toDegrees(elbowanglemax));
				}
		...
		if (jCheckBox2.isSelected()) {
			...
			try {
				if (!jTextPane3.getText().equals("")
						&& !jTextPane4.getText().equals("")) {
					super.shoulderUsed = true;
					super.shoulderanglemax = Math.toRadians(Integer
							.parseInt(jTextPane4.getText()));
					super.shoulderanglemin = Math.toRadians(Integer
							.parseInt(jTextPane3.getText()));
					System.out.println("Set Shoulder Angle Range from "
							+ Math.toDegrees(shoulderanglemin) + " to "
							+ Math.toDegrees(shoulderanglemax));
				}
			} catch (NumberFormatException ex) {
		...
		if (jCheckBox3.isSelected()) {
			...
			try {
				if (!jTextPane5.getText().equals("")
						&& !jTextPane6.getText().equals("")) {
					super.wristUsed = true;
					super.lrotateanglemax = Integer.parseInt(jTextPane6
							.getText());
					super.lrotateanglemin = Integer.parseInt(jTextPane5
							.getText());
					System.out.println("Set Wrist Angle Range from "
							+ lrotateanglemin + " to " + lrotateanglemax);
				}
			} catch (NumberFormatException ex) {
		...
		System.out.println("Angles will be tested:"
				+ (super.elbowUsed ? " elbow " : " ")
				+ (super.shoulderUsed ? " shoulder " : " ")
				+ (super.wristUsed ? " wrist " : " "));
